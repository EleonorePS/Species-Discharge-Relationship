...
a<-sdr[7,]#logq  coeffs not scaled


#marginal EF
#EF_marginal<-subset(d_pcrglob_subset,select = c('q','id_basin_pcrglob'))
EF_marginal_m<-a[['M4.coefficients']]/d_pcrglob_subset$q/(3600*24*365)# in PDF.m^-3.yr, warning! the model must fit non centered non scaled data 
EF_marginal2.5<-a[['CI.2.5..']]/d_pcrglob_subset$q/(3600*24*365)# in PDF.m^-3.yr, warning! the model must be fit n the not cetered and ot scaled vars
EF_marginal97.5<-a[['CI.97.5..']]/d_pcrglob_subset$q/(3600*24*365)# in PDF.m^-3.yr, warning! the model must be fit n the not cetered and ot scaled vars
...
EF_average$EF_average_m<-(1-(EF_average$qhuman/EF_average$qnatural)^a[['M4.coefficients']])/((EF_average$qnatural - EF_average$qhuman)*365*3600*24)
EF_average$EF_average2.5<-(1-(EF_average$qhuman/EF_average$qnatural)^a[['CI.2.5..']])/((EF_average$qnatural - EF_average$qhuman)*365*3600*24)
EF_average$EF_average97.5<-(1-(EF_average$qhuman/EF_average$qnatural)^a[['CI.97.5..']])/((EF_average$qnatural - EF_average$qhuman)*365*3600*24)
...
EF<-merge(EF_marginal, EF_average,by='id_basin_pcrglob', all=TRUE)

test=subset(EF,EF$EF_marginal_m<EF$EF_average_m)
test$ratio<-test$qhuman/test$qnatural
str(test)#1996 lines marginal < average, and 782 lines for marginal > average
...
print(quantile(EF$EF_average_m, seq(0,1,0.25),na.rm = TRUE), digits = 3)
mean(EF$ratio)
...
write.xlsx(export, "output/EF_patch.xlsx")#ok
...
dfdt<- read.xlsx("C:/Users/easpi/Documents/PhD Water Footprint/GLAM/code/output/depletion_factors_smooth_all_mix-1960-2000_20220316.xlsx")
...
write.xlsx(dfdt_patch, "output/DFDT_patch.xlsx")#ok'

dfdt_patch<- read.xlsx("output/DFDT_patch.xlsx")
...
#DISTRIBUTION
table<-quantile(CF$CF_average_m,c(0.0,0.25,0.5,0.75,1), na.rm = TRUE)
print(data.frame(table),digits = 3)
...
write.xlsx(CF, "output/CF_patch.xlsx", keepNA=TRUE)#ok'
