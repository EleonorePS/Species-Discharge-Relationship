...
KGE <- function(obs,sim,ex=2){
  ...
  if(length(obs) != length(sim)){
    ...
    if(sum(common_val,na.rm=T) != length(obs)){
      warning(paste0("\n",sum(obs_nas == F)," NA(s) found in obs",sum(sim_nas == F), " NA(s) found in sim"))
    }
    obs <- obs[common_val]
    sim <- sim[common_val]
...
results_model <- function(m, check){
  ...
  results<- data.frame(
    ...
    RootMeanSquareError = postResample(check_vals,log(check$SR_tot))[1],
    R2=postResample(check_vals,log(check$SR_tot))[2]
...
results_sim_obs <- function(sim,obs){
  ...
  #R2= R2(obs,sim)
  RootMeanSquareError = postResample(sim, obs)[1],
  R2=postResample(sim,obs)[2]
  )#maximize
...
plot(best_M$fitted.values, best_M$residuals)


sim50<-predict(best_M, d_pcrglob_preproc)#complete dataset
test50<-cbind(d_pcrglob_preproc, sim50)
...
print(results_cross_check, digits = 3)
