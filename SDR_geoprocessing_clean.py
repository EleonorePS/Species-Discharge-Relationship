...
#spatial_unit = tifffile.imread(Input.inputDir + '/' + 'gsim_BAF_MD.tif' )#gsim basin delineation
spatial_unit = ma.masked_values(spatial_unit, -3.40282e+38)#check  no data value

#create filter
index_filter=module.index_filter(spatial_unit)
pointer_array=index_filter[0]
idlist=index_filter[1]

#array containing the coordinates of the grid cells included in each river basin
#np.save(Input.inputDir +'/'+ 'filter_array_catchments_valerio_adjusted.npy', allow_pickle = 1) 
...
#idlist= idlist[3:,] #removing no data areas
...
ti_agg = module.aggregate_average(ti_map, pointer_array, area)#area is in km2!!!
h=module.plot_percentiles(ti_agg)

#discharge

#simulated data including water consumption
d  = Dataset(Input.outputDir + '/'+ 'discharge_outlet_human_1960_2010_pcrglob5.nc')#import raw data
...
q_yr = module.convert_month_to_year_sum(q*3600*24*365/12)#in m3/yr
...
d  = Dataset(Input.outputDir + '/'+ 'discharge_outlet_natural_1960-2010_pcrglob5.nc')#import raw data
...
qn_yr = module.convert_month_to_year_sum(qn*3600*24*365/12)#in m3/yr.
...
prec = tifffile.imread(Input.inputDir + '/' + 'wc2.1_5m_bio_12.tif' )#from world clim 2.1 #import raw data
...
temp = tifffile.imread(Input.inputDir + '/' + 'wc2.1_5m_bio_1.tif' ) #import raw data
#temp = tifffile.imread('D:/SDR/CHELSA_cur_V1_2B_r5m/5min/bio_1_resized.tif' )
temp = ma.masked_values(temp, -3.4e+38)
temp = temp + 273.15
...
prec_21_a = tifffile.imread(Input.inputDir + '/' + 'cclgmbi12.tif' ) #import raw data
prec_21_a = np.append(prec_21_a, np.full((2160-prec_21_a.shape[0],4320), -3.40282e+38), axis = 0)
prec_21_a = ma.masked_values(prec_21_a, -3.40282e+38)

prec_21_b = tifffile.imread(Input.inputDir + '/' + 'melgmbi12.tif' )#import raw data
...
prec_21_c = tifffile.imread(Input.inputDir + '/' + 'mrlgmbi12.tif' )#import raw data
prec_21_c = np.append(prec_21_c, np.full((2160-prec_21_c.shape[0],4320), -3.40282e+38), axis = 0)
prec_21_c = ma.masked_values(prec_21_c, -3.40282e+38)
...
temp_21_a = tifffile.imread(Input.inputDir + '/' + 'cclgmbi1.tif' )#import raw data
temp_21_a = np.append(temp_21_a, np.full((2160-temp_21_a.shape[0],4320), -3.40282e+38), axis = 0)
temp_21_a = ma.masked_values(temp_21_a, -3.40282e+38)
temp_21_a = temp_21_a/10 + 273.15 #in degree celsius

temp_21_b = tifffile.imread(Input.inputDir + '/' + 'melgmbi1.tif' )#import raw data
temp_21_b = np.append(temp_21_b, np.full((2160-temp_21_b.shape[0],4320), -3.40282e+38), axis = 0)
temp_21_b = ma.masked_values(temp_21_b, -3.40282e+38)
temp_21_b = temp_21_b/10 + 273.15 #in Kelvin

temp_21_c = tifffile.imread(Input.inputDir + '/' + 'mrlgmbi1.tif' )#import raw data
temp_21_c = np.append(temp_21_c, np.full((2160-temp_21_c.shape[0],4320), -3.40282e+38), axis = 0)
temp_21_c = ma.masked_values(temp_21_c, -3.40282e+38)
temp_21_c = temp_21_c/10 + 273.15 #in Kelvin


temp_21  = (temp_21_a  + temp_21_b + temp_21_c)/3
...
fer = tifffile.imread(Input.inputDir + '/' + 'FER.tif' )#import raw data
...
table = pd.read_excel(Input.inputDir + '/'+'freshwater_ecoregions.xlsx')#import raw data
...
table_sr = pd.read_excel(Input.inputDir + '/'+'pcrglobwb_5min_basins_SR.xlsx')#import raw data
...
df_f.shape
np.sum(df_f['area'])/np.sum(df_join['area'])#% landmass coverage
...
map_scope = module.make_map(mask, pointer_array, idlist, spatial_unit)
